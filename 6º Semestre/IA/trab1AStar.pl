...
peso([quarto(A,B)], E) :- estado_final([quarto(C, D)]), F is A-C, G is B-D, E is F+G, E >= 0. 
peso([quarto(A,B)], E) :- estado_final([quarto(C, D)]), F is A-C, G is B-D, H is F+G, H < 0, E is -H. 
...
pesquisa_aux([no(E,Pai,Op,C,CF,P)|_],no(E,Pai,Op,C,CF,P)) :- estado_final(E).

pesquisa_aux([E|R],Sol) :- expande(E,Lseg),
							insere_ordenado(Lseg,R,LFinal),
							pesquisa_aux(LFinal,Sol).
				
				  
expande(no(E,Pai,Op,C,CF,P),L) :- findall(no(En,no(E,Pai,Op,C,CF,P), Opn, Cnn, CFF, P1),
                				  (opr(E,Opn,En,Cn), P1 is P+1, Cnn is Cn+C,  peso(En, H), CFF is Cnn + H ), L).

pesquisa :- estado_inicial(S0),
