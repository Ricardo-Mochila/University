...
struct Div_admin * find(int cod_admin, struct Div_admin *array_admin[400])
{
	for (int i = 0; i < 400; ++i)
	{	
		if (array_admin[i] != NULL)
		{
			if (admin_code(array_admin[i]) == cod_admin)
			{

				return array_admin[i];
			}
		}
	}
...
{	
	...
	struct Div_admin ** mem_admin = initial_state(code_start, regiao, container_div);		
	...
	while(!is_empty(admin_queue))
	{
		...
		{
			relax(current, find(admin_ligacao(current, i), mem_admin));
		}
		admin_queue -> sorted = sort(num_divisoes, admin_queue -> sorted);	
	}

	admin_end = find(admin_code(admin_end), mem_admin);
